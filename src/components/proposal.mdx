*WIP proposal: last updated 8/10/2025*
...
over Cap'n Proto that allows database clients in a variety of languages to execute hybrid graph CRUD/semantic search transactions.
...
for efficient memory-mapped arrays and powerful parsing tooling (Menhir, Angstrom) for future query language extensions. The set of OxCaml extensions
will also be useful for performance improvements as an extension, through allocation control and safe concurrency.
...
I have limited experience with OCaml, exclusive to knowledge from the CST courses. An existing set of LMDB bindings in OCaml,
...
- Performance is close to existing graph-only and vector-only peers for their respective operations.
